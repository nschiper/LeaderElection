...
extern void send_accusation(struct sockaddr_in *raddr, u_int gid);
extern void accusation_merge(char *msg, int msg_len, struct sockaddr_in *cliAddr);

/* Omega algorithm module */
...
extern struct list_head actives_head;
...
extern inline int add_proc_in_actives_set(struct sockaddr_in *addr, u_int pid, u_int gid);
extern inline int remove_proc_from_actives_set(struct sockaddr_in *addr, u_int pid, u_int gid);
extern inline int updateLocalLeader(unsigned int gid);
extern inline int updateGlobalLeader(unsigned int gid);
extern int updateGlobalLeaders();
extern inline void doUponSuspected(struct sockaddr_in *addr, u_int pid, u_int gid, struct timeval *now);
extern inline void doUponReceivedAccusation(u_int gid);
...
extern int sched_report_group_now(int gid, struct timeval *now);
