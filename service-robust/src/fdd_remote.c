...
static char *parse_and_insert_remotevars_list(char *msg, char *ptr_start, unsigned int remotevars_count,
  ...
  for(i=0 ; i < remotevars_count ; i++) {
    ptr = msg_parse_rep_localvars(ptr, &gid, &accusationTime, &bestAmongActives,
    ...
    *retval = -EMSGSIZE ;
    ...
    if (omega_group_exists_locally(gid, NOT_CANDIDATE)) {
      if (insert_in_remotevars(addr, gid, &accusationTime, &bestAmongActives,
  ...
  for(i = 0 ; i < pcount ; i++) {
    ...
    *retval = -ENOMEM ;
    ...
    if (omega_group_exists_locally(gid, NOT_CANDIDATE) && eta_rcvd) {
      if (add_proc_in_actives_set(addr, pid, gid) < 0) {
        fprintf(stderr, "omega error: upon receiving alive msg from: %u.%u.%u.%u impossible\n",  NIPQUAD(addr));
        fprintf(stderr, "to add proc: %u in actives set of group: %u\n", pid, gid);
      }
    }
  }
  *retval = 0 ;
...
extern void remote_merge(char *msg, int msg_len,
  ...
  he finished the INITIAL. He couldn't have sent this message if he
  didn't have finish the INITIAL phase. So I declare the INITIAL
  ...
  if(
    ...
    for(i = 0 ; i < remote_groups_count ; i++) {
      ...
      &remote_group_procs_list, eta_rcvd, &retval) ;
