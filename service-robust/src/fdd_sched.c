...
/* sched.c - schedule events for the future - reports,
...
/* sched_report - schedule a report message now! */
/* insert an EVENT_REPORT in the list of reports for all hosts in group gid */
extern int sched_report_group_now(int gid, struct timeval *now) {
  
  struct list_head *tmp_host, *tmp_group;
  struct host_struct *host;
  ...
  list_for_each(tmp_host, &remote_host_list_head) {
    ...
    list_for_each(tmp_group, &host->jointly_groups_head) {
      group = list_entry(tmp_group, struct uint_struct, uint_list);
      if (group->val == gid) {
        remove_report_event(host);
        retval = add_event(EVENT_REPORT, NULL, NULL, NULL, host, NULL, now);
        if (retval < 0)
          return retval;
        break;
      }
