...
static int omega_send_ext_result(struct localregistered_proc_struct *rproc, int result,
  ...
  char msg[OMEGA_FIFO_MSG_LEN] ;
  msg_omega_build_ext_res(msg, result, &leader->addr, leader->pid) ;
  return write_msg(rproc->omega_qry_fd, msg, OMEGA_FIFO_MSG_LEN) ;
}


/* Removes the leader variable of a group gip if there are no more local processes in
gid. It also either deletes the actives set of gid for the same reason as above
...
static void cleanup_leader_and_contenders(struct localregistered_proc_struct *rproc,
  unsigned int gid, int candidate) {
  
  struct list_head *tmp_head1, *tmp_head2;
  struct contenders_struct *tmp_actives, *tmp_contenders;
  ...
  if (local_group_empty) {
    ...
    list_for_each(tmp_head1, &actives_head) {
      ...
      if (tmp_actives->gid < gid)
        ...
        list_del(&tmp_actives->contenders_list);
        list_free(&tmp_actives->procs_list, struct proc_struct, proc_list);
        free(tmp_actives);
        break;
      }
      else
    ...
    /* Remove the process from the actives set. If the process
     is not a candidate, nothing will be done.*/
    remove_proc_from_actives_set(&omega_localaddr, rproc->pid, gid);
  ...
  /* If the local group was empty we would already have deleted the actives set, so there
...
static void omega_local_reg(char *msg, fd_set *dset) {
...
static void omega_local_unreg(struct localregistered_proc_struct *rproc, fd_set *dset, struct timeval *now) {
  ...
  list_for_each(tmp_head, &rproc->notif_type_list) {
    ...
    free(tmp_notif);
    cleanup_leader_and_contenders(rproc, gid, candidate);
  }
...
static void do_startOmega(struct localregistered_proc_struct *rproc, char *msg, struct timeval *now) {
  ...
  if (candidate == CANDIDATE) {
    if (do_join_group(rproc->pid, gid, now) < 0) {
#ifdef OMEGA_OUTPUT
      fprintf(stderr, "omega: join group: %u failed\n", gid) ;
#endif
#ifdef OMEGA_LOG
      fprintf(omega_log, "omega: join group: %u failed\n", gid) ;
#endif
      retval = -1;
      goto out;
    }
...
extern void check_omega_fifo(fd_set *active, fd_set *dset) {
  char msg[OMEGA_FIFO_MSG_LEN] ;
  ssize_t len ;
