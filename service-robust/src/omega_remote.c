...
void accusation_merge(char *msg, int msg_len, struct sockaddr_in *cliAddr) {
  
  u_int gid;
  
  msg_omega_parse_accusation(msg, &gid);
  
  doUponReceivedAccusation(gid);
  
#ifdef OMEGA_OUTPUT
  fprintf(stdout, "Received accusation from: %s for gid: %u\n",
  inet_ntoa((*cliAddr).sin_addr), gid);
#endif
#ifdef OMEGA_LOG
  fprintf(omega_log, "Received accusation from: %s for gid: %u\n",
  inet_ntoa((*cliAddr).sin_addr), gid);
#endif
}


/* check_socket - reads a msg from the TCP socket */
void check_omega_socket(fd_set *active, struct timeval *now) {
  ...
  switch (msg_type(msg)) {
    ...
    accusation_merge(msg, msg_len, &cliAddr);
    break ;
